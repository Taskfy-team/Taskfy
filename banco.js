...
async function buscarUsuario(usuario) {
  ...
  const [usuarioEncontrado] = await conexao.query(sql, [
    usuario.email,
    usuario.senha,
  ]);
...
async function createGrupo(grupo) {
  ...
  const sql =
    "INSERT INTO equipes(nome_equipe, desc_equipe, status_equipe, donoEquipe) VALUES (?, ?, 'ativo', ?);";
...
async function createtarefa(tarefa) {
  ...
  const sql =
    "INSERT INTO tarefas(nome_tarefa, descricao_tarefa, status_tarefa, fk_dono_tarefa, fk_equipe, dataCriacao, prazoTarefa) VALUES (?, ?, ?, ?, ?, ?, ?);";
...
async function cadastrarusu(usuario) {
  ...
  const sqlemail =
    "SELECT COUNT(*) AS veremail FROM usuario WHERE email_usuario = ?;";
...
async function buscargrupo(userid) {
  const conexao = await conectarBD();
  const sql =
    "SELECT * FROM equipes INNER JOIN usuario ON usuario.id_usuario = equipes.donoEquipe WHERE id_equipe = ?;";
  const [rows] = await conexao.query(sql, [userid]);
  return rows;
}
...
async function pertencegrupo(grupo) {
  const conexao = await conectarBD();
  const sql =
    "SELECT COUNT(*) AS pertence FROM usuario_equipe WHERE fk_usuario = ? AND fk_equipe = ?;";
...
async function pertencetarefa(grupo) {
  const conexao = await conectarBD();
  const sql =
    "SELECT COUNT(*) AS pertence FROM usuario_tarefa WHERE fk_usuario = ? AND fk_tarefa = ?;";
...
async function atualizardadosusr(usr) {
  const conexao = await conectarBD();
  const sql =
    "UPDATE usuario SET nome_usuario = ?, senha_usuario = ? WHERE id_usuario = ?;";
...
async function buscarcolabgrupo(idGrupo) {
  const conexao = await conectarBD();
  const sql =
    "SELECT usuario.email_usuario AS email FROM usuario_equipe  INNER JOIN usuario on usuario.id_usuario = usuario_equipe.fk_usuario WHERE fk_equipe = ?;";
...
async function removercolabgrupo(params) {
  ...
  const [equipesremovidas] = await conexao.query(removerdasequipes, [
    params.grupo,
    params.email,
  ]);
...
async function excluirgp(params) {
  ...
  await conexao.query(`DELETE FROM tarefas WHERE fk_equipe = ?;`, [
    params.grupo,
  ]);

  await conexao.query(`DELETE FROM usuario_equipe WHERE fk_equipe = ?;`, [
    params.grupo,
  ]);
...
async function buscarcolabtarefa(idTarefa) {
  const conexao = await conectarBD();
  const sql =
    "SELECT usuario.email_usuario AS email FROM usuario_tarefa  INNER JOIN usuario on usuario.id_usuario = usuario_tarefa.fk_usuario WHERE fk_tarefa = ?;";
...
async function enviarmsgtimeline(params) {
  ...
  if (params.status !== "NA") {
    ...
    const [rowstatus] = await conexao.query(sqlstatus, [
      params.status,
      params.tarefa,
    ]);
  }
...
async function inserirNovoUsuarioOuAdmin(nome, email, senha, tipo) {
  ...
  if (tipo === "admin") {
    const sql =
      "INSERT INTO admin (nome_admin, email_admin, senha_admin) VALUES (?, ?, ?)";
    await conexao.query(sql, [nome, email, senha]);
  } else {
    const sql =
      'INSERT INTO usuario (nome_usuario, email_usuario, senha_usuario, status) VALUES (?, ?, ?, "ativo")';
    await conexao.query(sql, [nome, email, senha]);
  }
}
